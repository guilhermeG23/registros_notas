...
create table if not exists Marca_Maquina (
...
create table if not exists Software_Microsoft (
	...
	foreign key (Nota_Fiscal_Ex) references Nota_Fiscal(Nota_Fiscal),
...
create table if not exists Maquina (
	...
	ID_Marca_Ex int not null,
	Modelo varchar(100) not null,
	Nota_Fiscal_Ex varchar(44) not null,
	ID_Setor_Ex int(2) not null,
	Chapa_Func_Ex int(9) not null,
	foreign key (ID_Marca_Ex) references Marca_Maquina(ID_Marca),
	foreign key (Nota_Fiscal_Ex) references Nota_Fiscal(Nota_Fiscal),
	foreign key (ID_Setor_Ex) references Setor(Setor_ID),
	foreign key (Chapa_Func_Ex) references Funcionario(Chapa)
);
...
insert into Marca_Maquina values (default, "Lenovo"), (default, "Samsung"), (default, "Dell"), (default, "HP");
...
#Select
create view vw_software as select Nota_Fiscal.Nota_Fiscal as Nota, Software_Microsoft_Modelos.Versao as Versao, Tipos_Sistemas.Sistema as Tipo from Nota_Fiscal inner join Software_Microsoft on Software_Microsoft.Nota_Fiscal_Ex = Nota_Fiscal.Nota_Fiscal inner join Software_Microsoft_Modelos on Software_Microsoft.ID_Modelo_Ex = Software_Microsoft_Modelos.ID_Modelo inner join Tipos_Sistemas on Software_Microsoft_Modelos.ID_Tipo_Ex = Tipos_Sistemas.ID_Tipo;

create view vw_marca_arrumada as select * from Marca_Maquina order by Marca asc;
